  ...
  def draw(width, height, robot_pos, path_length)
    ...
    map_lines = y_range.map do |y|
      x_range.map do |x|
        pos = Pos.new(x, y)
        cell = tile(pos)
        cell = ROBOT if robot_pos == pos && ![OXYGEN, MARKED, FILLED_OXYGEN].include?(cell)
...
class GameState
  ...
  def bfs_for_oxygen
    bfs(Pos.new(0, 0)) do |pos|
      @grid.mark(pos, Grid::MARKED)

      break if pos == oxygen_pos
    end
  end

  def bfs_to_fill_oxygen
    @grid.mark(oxygen_pos, Grid::FILLED_OXYGEN)

    bfs(oxygen_pos) do |pos|
      @grid.mark(pos, Grid::FILLED_OXYGEN)
    end
  end

  private def oxygen_pos
